...
log_dir = './checkpoint/log50-CIFAR100/'
os.makedirs(log_dir, exist_ok=True)  # Create the directory if it doesn't exist

# Define the file path where you want to save the logs
log_file = os.path.join(log_dir, 'training_logs50.txt')
...
if args.resume:
    ...
    checkpoint = torch.load('./checkpoint/CIFAR100-resnet50.pth')
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/CIFAR100-resnet50.pth')
        best_acc = acc
